...
export function activate(context: ExtensionContext) {
	...
	let config: Record<string, any> = JSON.parse(
		JSON.stringify(workspace.getConfiguration("htmx-lsp2"))
	);
	...
	const clientOptions: LanguageClientOptions = {
		...
		documentSelector: [{ scheme: 'file', language: 'jinja-html' }, { scheme: 'file', language: 'rust' }, { scheme: 'file', language: 'html' }],
		initializationOptions: config,
		synchronize: {
			// Notify the server about file changes to '.clientrc files contained in the workspace
			fileEvents: workspace.createFileSystemWatcher('**/.{jinja, html, rs}')
		}
	};
...
function getServer(): { valid: boolean, name: string } {
	try {
		// let name = "/home/uros/.cache/cargo/target/debug/htmx-lsp2";
		let name = "htmx-lsp2";
		...
		if (validation.status === 0) {
			...
			return { valid: false, name: "HTMX language server is not installed." }
		}
