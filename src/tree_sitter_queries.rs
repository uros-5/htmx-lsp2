...
use crate::tree_sitter_htmx::Position;
...
pub fn query_attr_name_for_completion(
    node: Node<'_>,
    source: &str,
    trigger_point: Point,
) -> Option<Position> {
...
pub fn query_attr_values_for_completion(
    ...
    if let Some(capture) = props.get("non_empty_attribute") {
        let attr_value_temp = props.get("attr_value");
        if let Some(attr_name) = attr_value_temp {
            attr_value = attr_name.value.as_str();
        } else {
