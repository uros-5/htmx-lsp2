use crate::tree_sitter_queries::{
    query_attr_name_for_completion, query_attr_values_for_completion,
};
use dashmap::DashMap;
use log::{debug, error};
use ropey::Rope;
...
fn query_position(root: Node<'_>, source: &str, trigger_point: Point) -> Option<Position> {
    ...
    let attr_completion = query_attr_name_for_completion(element, source, trigger_point);
